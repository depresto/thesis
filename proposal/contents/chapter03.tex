...
\begin{enumerate}
  ...
  \item The carpooling fare is shared equally by the time passengers in the vehicle. For a certain section of the route that has $n$ passengers, this fare of link for a passenger will be divided by $n$.
 \end{enumerate}
...
\begin{table}[ht]
  ...
  \midrule
    ...
    $L_p$ & Set of links on the road network that pass by the passenger $p \in P$ \\
    ...
    $P_{rp_i}$ & Set of all paths for driver $r$ from initial location to passenger $p_i$, where $r \in R$ and $p_i \in P$ \\
...
\begin{table}[ht]
  ...
  Notation & Description \\
  \midrule
    $x_p$ & Binary variable, 1 if route $p \in P_r$ is chosen; 0 otherwise. \\
    $y_l$ & Binary variable, 1 if link $l \in L$ is on the shortest path; 0 otherwise. \\
...
\begin{align*}
  max\ min\ \frac{Original\ cost_i - carpool\ cost_i}{Original\ cost_i} \tag{IP1}
\end{align*}
...
\begin{align*}
  ...
  & \sum\limits_{p \in P_r} x_p \delta_{pl} \leq y_l && \forall l \in L \cup L_p \cup L_d \tag{3.2} \\
  ...
  & y_l \leq 1 && \forall l \in L_p \cup L_d \tag{3.3} \\
  ...
  & \sum\limits_{p \in P_w} z_p \delta_{pl} \leq y_l && \forall w \in W \tag{3.4} \\
  ...
  & \sum\limits_{l \in L \cup L_p} \sum\limits_{p \in P_{rp_i}} z_p \delta_{pl} f_l \leq \sum\limits_{l \in L \cup L_d} \sum\limits_{p \in P_{rd_i}} z_p \delta_{pl} f_l && \forall i \in \{1,2,,...,n\}, r \in D \tag{3.6} \\
